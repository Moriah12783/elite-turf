...
export default function ModifierCoursePage() {
  ...
  useEffect(() => {
    ...
    supabase.from("courses").select("*").eq("id", id).single()
      .then(({ data, error }) => {
        ...
        setForm({
          hippodrome_id: data.hippodrome_id || "",
          date_course: data.date_course || "",
          heure_depart: (data.heure_depart || "").substring(0, 5),
          numero_reunion: data.numero_reunion || 1,
          numero_course: data.numero_course || 1,
          libelle: data.libelle || "",
          distance_metres: data.distance_metres || 1600,
          categorie: data.categorie || "PLAT",
          terrain: data.terrain || "BON",
          nb_partants: data.nb_partants || 10,
          statut: data.statut || "PROGRAMME",
        });
        setParisDisponibles(Array.isArray(data.paris_disponibles) ? data.paris_disponibles : []);
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-text-muted text-xs mt-0.5 font-mono truncate max-w-[200px]">{id}</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="card-base p-5 space-y-4">
          <div className="flex items-center gap-2 border-b border-border pb-3">
            ...
            <span className="text-text-muted text-xs ml-auto">
              {parisDisponibles.length} sélectionné(s)
            </span>
          </div>
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link
            href="/admin/courses"
            className="px-5 py-3 text-text-secondary hover:text-text-primary text-sm font-medium transition-colors"
          >
            Annuler
          </Link>
        </div>
