...
import {
  syncCoursesToDB,
  type GenyCourse,
  type GenyProgrammeClient,
} from "./geny-programme";

/**
 * Écriture du programme par la voie GENY (`syncCoursesToDB`), distincte de la
 * voie de secours partagée `programme-upsert.ts` (PMU/LONACI/GenyBet).
 *
 * ⚠️ Ce qui est testé ici est la REMONTÉE DES ÉCHECS, pas le scraping : le
 * `{ error }` de supabase-js n'était lu dans AUCUNE des 5 requêtes, donc un
 * INSERT refusé (NOT NULL, overflow numérique, RLS…) renvoyait quand même
 * `inserted: N`. `loadGenyProgramme` en concluait que le programme était chargé
 * et renvoyait `source: "geny"` avec un compte fabriqué : la CLI/cron voyait un
 * succès, personne n'était alerté, et le programme du jour restait vide.
 *
 * Le client Supabase est injecté (`opts.client`) pour exercer le VRAI
 * `syncCoursesToDB` — câblage compris — sans base de données.
 */
...
function fakeSupabase(cfg: FakeConfig): { client: GenyProgrammeClient; calls: CallLog } {
  const calls: CallLog = [];
  ...
  return { client: client as unknown as GenyProgrammeClient, calls };
}

// ── Données ─────────────────────────────────────────────────────────────────

function course(over: Partial<GenyCourse> = {}): GenyCourse {
  return {
    reunionNum:       1,
    hippodromeNom:    "Vincennes",
    ...
    genyUrl:          "/partants-pmu/2026-07-25-vincennes-pmu-prix-de-test_c1",
    ...over,
  };
}

/** La voie Geny résout les hippodromes par `nom` exact (pas de canonicalisation). */
const VINCENNES = { id: "hip-vincennes", nom: "Vincennes" };
...
describe("syncCoursesToDB — remontée des échecs d'écriture", () => {
  it("propage l'erreur de l'INSERT des courses au lieu de renvoyer un faux succès", async () => {
    ...
    await expect(
      syncCoursesToDB([course({ courseNum: 1 }), course({ courseNum: 2 })], { client }),
  ...
  it("propage l'erreur de l'UPSERT des courses existantes", async () => {
    ...
    await expect(
      syncCoursesToDB([course({ courseNum: 1 })], { client }),
    ).rejects.toThrow(/permission denied/);
  });

  it("propage l'erreur de l'INSERT des hippodromes (sinon les courses sont ignorées en silence)", async () => {
    // Hippodrome absent de la base → INSERT refusé. Sans remontée, `hipMap`
    // reste vide, toutes les courses sont `continue`-ées et la fonction renvoie
  ...
  it("propage l'erreur du SELECT des hippodromes (sinon on recrée des doublons)", async () => {
    ...
    // passeraient pour manquants et on tenterait de les recréer.
    const { client, calls } = fakeSupabase({
    ...
    expect(calls).not.toContain("hippodromes.insert(1)");
  });

  it("propage l'erreur du SELECT des courses existantes (sinon on ré-insère des doublons)", async () => {
    // `data: null` → aucune course connue → tout repart en INSERT, y compris les
    // courses déjà présentes.
    const { client, calls } = fakeSupabase({
      hippodromesSelect: { data: [VINCENNES], error: null },
...
describe("syncCoursesToDB — comptage fidèle", () => {
  it("compte les lignes réellement écrites (count Supabase), pas celles envoyées", async () => {
    ...
    const r = await syncCoursesToDB(
      [course({ courseNum: 1 }), course({ courseNum: 2 }), course({ courseNum: 3 })],
      { client },
    );
  ...
  it("écriture réussie sans count renvoyé → compte les lignes envoyées", async () => {
    ...
    const r = await syncCoursesToDB(
      [course({ courseNum: 1 }), course({ courseNum: 2 })],
      { client },
    );
  ...
  it("hippodrome créé à la volée : son id sert à insérer les courses", async () => {
    const { client } = fakeSupabase({
      hippodromesSelect: { data: [], error: null },
      hippodromesInsert: { data: [VINCENNES], error: null },
      coursesInsert:     { error: null, count: 1 },
    });

    const r = await syncCoursesToDB([course()], { client });

    expect(r).toEqual({ inserted: 1, updated: 0, hippodromes: 1 });
  });

  it("liste vide → aucun appel à la base", async () => {
    const { client, calls } = fakeSupabase({});
    const r = await syncCoursesToDB([], { client });
